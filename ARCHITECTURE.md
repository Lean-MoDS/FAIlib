...
FAIlib follows a subject-first filesystem modeled on Mathlib. Every Lean module lives below one of
seven layer-one directories; there are no Lean files directly in `FAIlib/`.
...
- Do not add `Main.lean`, `Infrastructure.lean`, or an unqualified `Defs.lean` under `FAIlib/`.
...
- A whole-library umbrella, if one is ever needed, must be a declaration-free `FAIlib.lean`
  outside `FAIlib/`; topic modules should remain selectively importable.
