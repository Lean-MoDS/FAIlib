...
`FAIlib/` contains 89 Lean modules under seven layer-one directories and no root-level Lean
files. A filesystem path such as `FAIlib/Probability/Process/Dudley.lean` corresponds to the Lean
module `FAIlib.Probability.Process.Dudley`.
...
Only the public `FAIlib/` source tree is shown. Historical provenance is recorded in file headers
