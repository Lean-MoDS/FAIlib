...
import FAIlib.Probability.Moments.Cumulant
...
This module uses Mathlib's moment and cumulant generating functions.
...
theorem hoeffding [IsProbabilityMeasure μ] (t a b : ℝ) {X : Ω → ℝ} (hX : AEMeasurable X μ)
  ...
  case neg =>
    ...
    · filter_upwards [h] with ω hω
      simp only [Set.mem_Icc, Pi.neg_apply]
      exact ⟨neg_le_neg hω.2, neg_le_neg hω.1⟩
    · simpa only [Pi.neg_apply, integral_neg, neg_eq_zero] using h0
