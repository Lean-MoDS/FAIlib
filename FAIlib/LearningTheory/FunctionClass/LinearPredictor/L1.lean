...
import FAIlib.LearningTheory.Rademacher.Signs
import FAIlib.Analysis.NormedSpace.CoveringNumber.L1
...
* `l1Norm`: compatibility view of FAIlib's canonical `l1norm`.
* `L1Ball`: subtype associated with FAIlib's canonical `l1Ball`.
...
-- Compatibility spelling for the canonical FAIlib ℓ1 norm.
noncomputable abbrev l1Norm (w : EuclideanSpace ℝ (Fin d)) : ℝ :=
  l1norm w

-- Subtype of the canonical FAIlib ℓ1 ball.
def L1Ball (W : ℝ) : Type :=
  ↥(l1Ball (d := d) W)
